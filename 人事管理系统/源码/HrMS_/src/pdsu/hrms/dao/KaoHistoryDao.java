...
public class KaoHistoryDao {
	...
	public static String[] getDeptsForSelect() {
		...
		try {
			// 查询
			String sql = "select distinct assess from person";
...
//				person.setDept2Name(rs.getString("dept2Name"));
				list.add(person);
			}
			// 把list转换为二维数组
			data = new String[list.size()];
			for (int i = 0; i < list.size(); i++) {
				data[i] = list.get(i).getAssess();
			}
		} catch (Exception e) {
...
//				System.out.println(person.getDeptId());
				...
				ps.setString(1, person.getAssess());
				ps.executeUpdate();
				conn.commit();
		...
		public static String[][] getHistoryPersonForTable() {
			...
			try {
				// 查询
				String sql = "select histId,oldInfo,newInfo,chgDate,personId,chgNum from histroy where histType like '人员考核'";
				ps = conn.prepareStatement(sql);
				rs = ps.executeQuery();
				// 处理结果集
				while (rs.next()) {
...
//					
		...
		public static long getNextId(Person person) {
			...
			ResultSet rs = null;
			int id=0;
			String sql = "select max(chgNum) from histroy where histType like '人员考核' and personId=(?)";
			try {
				ps = conn.prepareStatement(sql);
